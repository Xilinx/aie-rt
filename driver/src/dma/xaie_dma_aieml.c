...
/**
*
* This API prepares the buffer descriptor words for an AIEML Memory Tile DMA BD.
*
...
*			XAIEML_MEMTILEDMA_NUM_BD_WORDS entries).
		...
		XAie_LocType Loc, u8 BdNum)
{
	...
	RC = _XAieMl_MemTileDmaWriteBdPrep(DevInst, DmaDesc, Loc, BdNum,
					    BdWord, &Addr);
	if (RC != XAIE_OK) {
		return RC;
	}
...
/**
*
* This API prepares the buffer descriptor words for an AIEML Tile DMA BD.
*
* @param	DevInst: Device Instance
* @param	DmaDesc: Initialized Dma Descriptor.
* @param	Loc: Location of AIE Tile
* @param	BdNum: Hardware BD number to be written to.
* @param	BdWord: Output buffer for BD words (must hold at least
*			XAIEML_TILEDMA_NUM_BD_WORDS entries).
* @param	Addr: Output pointer to store the register address to write to.
*
* @return	XAIE_OK on success, Error code on failure.
*
		...
		XAie_LocType Loc, u8 BdNum)
{
	...
	RC = _XAieMl_TileDmaWriteBdPrep(DevInst, DmaDesc, Loc, BdNum,
					 BdWord, &Addr);
	if (RC != XAIE_OK) {
		return RC;
	}
...
/**
*
* This API prepares the buffer descriptor words for an AIEML Shim DMA BD.
*
* @param	DevInst: Device Instance
* @param	DmaDesc: Initialized Dma Descriptor.
* @param	Loc: Location of AIE Tile
* @param	BdNum: Hardware BD number to be written to.
* @param	BdWord: Output buffer for BD words (must hold at least
*			XAIEML_SHIMDMA_NUM_BD_WORDS entries).
* @param	Addr: Output pointer to store the register address to write to.
*
* @return	XAIE_OK on success, Error code on failure.
*
		...
		XAie_LocType Loc, u8 BdNum)
{
	...
	RC = _XAieMl_ShimDmaWriteBdPrep(DevInst, DmaDesc, Loc, BdNum,
					 BdWord, &Addr);
	if (RC != XAIE_OK) {
		return RC;
	}
